...
const account2 = {
  ...
  movementsDates: [
    ...
    '2025-05-25T18:49:59.371Z',
    '2025-06-22T12:01:20.894Z',
  ],
  pin: 2222,
  type: 'standard',
};
...
const formatMovementsDate = function (date) {
  ...
  if (daysPassed === 0) return `Today , ${hour}:${minutes}`;
  if (daysPassed === 1) return `Yesterday , ${hour}:${minutes}`;
  if (daysPassed <= 7) return `${daysPassed} days ago , ${hour}:${minutes}`;
  else {
...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const dispalyDate = formatMovementsDate(date);
    const html = `
...
const startLogOutTimer = function () {
  ...
  const tick = function () {
    ...
    //When time becomes 0 , stop timer and LogOut user
    if (time === 0) {
      clearInterval(timer);
...
//For Balance time to get updated every second
const updateTime = function () {
  ...
  labelDate.textContent = `${date}/${month}/${year} , ${hour}:${minutes}:${seconds}`;
};
updateTime();
